  ...
  try {
    ...
    if (response.code === 200 && response.data?.token) {
      ...
      logger.info('认证信息已保存:', {
        ...
        token格式: token ? `长度=${token.length}, 部分=${token.split('.').length}` : '无效',
      ...
      if (!authStore.isAuthenticated) {
        logger.error('认证状态无效，登录失败，token:', token ? '存在' : '不存在')
        logger.error('isLoggedIn:', authStore.isLoggedIn)
        logger.error('token值:', authStore.getToken ? authStore.getToken.substring(0, 50) + '...' : 'null')
        success.value = false

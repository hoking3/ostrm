...
const getConfigs = async () => {
  console.log('[getConfigs] 开始获取配置')
  console.log('[getConfigs] 当前认证状态:')
  console.log('  - isLoggedIn:', authStore.isLoggedIn)
  console.log('  - token存在:', !!authStore.getToken)
  console.log('  - token值:', authStore.getToken?.substring(0, 50) + '...' || 'null')

  loading.value = true
  try {
    console.log('[getConfigs] 发送API请求...')
    const response = await authenticatedApiCall('/openlist-config', {
      method: 'GET'
    })
    console.log('[getConfigs] 收到响应:', response)

    if (response.code === 200) {
      configs.value = response.data || []
      console.log('[getConfigs] 配置加载成功，数量:', configs.value.length)
    } else {
      logger.error('获取配置列表失败:', response.message)
    }
  } catch (error) {
    console.error('[getConfigs] 获取配置列表错误:', error)
    logger.error('获取配置列表错误:', error)
  } finally {
    loading.value = false
    console.log('[getConfigs] 完成, loading=false')
  }
}
...
// 组件挂载时初始化认证状态和获取配置列表
onMounted(async () => {
  console.log('[Index Page] onMounted 开始执行')
  console.log('[Index Page] localStorage中的token:', localStorage.getItem('auth_token')?.substring(0, 50) + '...' || '不存在')

  // 初始化认证状态
  authStore.restoreAuth()
  console.log('[Index Page] restoreAuth完成后, isLoggedIn:', authStore.isLoggedIn)
  console.log('[Index Page] store中的token:', authStore.getToken?.substring(0, 50) + '...' || '不存在')

  // 延迟一下再获取配置，确保认证状态完全恢复
  await new Promise(resolve => setTimeout(resolve, 100))

  console.log('[Index Page] 准备调用getConfigs')
  await getConfigs()
  console.log('[Index Page] getConfigs完成, configs数量:', configs.value.length)
})
</script>
